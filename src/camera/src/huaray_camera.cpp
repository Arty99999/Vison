...
//            cv::rotate(bgr24Image,bgr24Image,cv::ROTATE_90_CLOCKWISE);
//            cv::rotate(bgr24Image,bgr24Image,cv::ROTATE_180);
...
void HuarayCamera::RecordVideo()
{
    ...
    uint64_t pretimeStamp = this->GetOpenTimestamp();
    // 录制相机数据帧
    while (recordVideoSwitch_)
    {
        ...
        uint64_t Timegap = (endTimestamp - pretimeStamp)/ 1000000000;
//        std::cout << Timegap <<std::endl;
//        std::cout << samplePeriod <<"    " <<  writeSpan << std::endl;
        if (Timegap > 60) {
            pretimeStamp = endTimestamp;
            videoWriter_.release();
        }
        ...
        if (!videoWriter_.isOpened()){
            ...
            if (videoWriter_.open(fullPath,
                                  ...
                                  true))
            {
                std::cout << "Camera was started recording successful" << std::endl;
            }else
                std:: cout << " Camera was opened failure because it can not be started recording"  << std::endl;
