...
int main(int argc, char *argv[])
{
    ...
    while (true)
    {
        ...
        if (isLogoReady && isFanReady)
        {
            ...
            // 对风车扇叶进行速度补偿，并更新击打目标点数据缓冲区
            if (solver.CompensateBuffVelocity(EWorkMode::ShootLargeBuff,
                                              logoLocation,
                                              fanLocationSequence,
            ...
            // 搜索匹配的历史击打目标点集合
            uint64_t prediceTime = solverParam.LargeBuffVelocityParameter.PredictTime * 1000000;
