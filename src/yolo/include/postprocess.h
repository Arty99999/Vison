...
__inline__ void scale_bbox(const cv::Mat& img, float bbox[4], int kInputW, int kInputH){
...
__inline__ std::vector<std::vector<float>> scale_kpt_coords(const cv::Mat& img, float* pkpt, int kInputW, int kInputH , int kNumKpt, int kKptDims){
    float r_w = kInputW / (img.cols * 1.0);
    float r_h = kInputH / (img.rows * 1.0);
    float r = std::min(r_w, r_h);
    float pad_h = (kInputH - r * img.rows) / 2;
