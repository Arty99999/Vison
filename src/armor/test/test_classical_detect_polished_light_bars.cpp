//
// Created by plutoli on 2022/1/14.
//
...
int main(int argc, char *argv[])
{
    ...
    cv::namedWindow("polishedLightBarsImage", cv::WINDOW_NORMAL);
    cv::resizeWindow("polishedLightBarsImage", 800, 600);
    ...
    while (true)
    {
        ...
        // 创建精灯条图像
        cv::Mat polishedLightBarsImage;
        ClassicalArmorRecognizer::CreateLightBarsImage(polishedLightBars,
                                                       cameraData.Image,
                                                       &polishedLightBarsImage);

        // 显示图像
        cv::imshow("rawImage", cameraData.Image);
        cv::imshow("binaryImage", binaryImage);
        cv::imshow("polishedLightBarsImage", polishedLightBarsImage);
