...
int main(int argc, char *argv[])
{
    ...
    std::string sampleNegativePath = "generator/armor/";
    ...
    cv::imshow("grayScaleImage", bigImage);
    ...
    while (camera.IsOpened())
    {
        ...
        // 按照装甲板Hog特征窗体大小依次对原始图像进行分割
        int rowNumber = grayScaleImage.rows / recognizerParam.ArmorHogWindowHeight;
        int colNumber = grayScaleImage.cols / recognizerParam.ArmorHogWindowWidth;
        for (int i = 0; i < colNumber; ++i)
        {
            for (int j = 0; j < rowNumber; ++j)
            {
                // 提取负样本图片
                cv::Rect rect(i * recognizerParam.ArmorHogWindowWidth,
                              j * recognizerParam.ArmorHogWindowHeight,
                              recognizerParam.ArmorHogWindowWidth,
                              recognizerParam.ArmorHogWindowHeight);
