...
// 对 y = -(a/w) * cos(w * t + fai) + (2.09 -a) * t + (a/w) * cos(fai) 进行采样和拟合
int main(int argc, char *argv[])
{
    ...
    for (unsigned i = 0; i < 500; ++i)
    {
        ...
        auto y = static_cast<float>(-(a / w) * cos(w * t + fai) + (2.09 - a) * t + (a / w) * cos(fai));
    ...
    Solver::FitSineIntegral(datas, 50, initialCoeffs, &fittedCoeffs);
