...
bool SerialPort::Open()
{
    ...
    if (!SearchDriverFiles(param_.Name, &driverFiles))
    {
        log = "[" + param_.Key + "] - SerialPort can not be opened because driverFiles were searched failure";
        logger.Save(ELogType::Error, log);
        log = LOG_END;
        logger.Save(ELogType::Info, log);
        return false;
    ...
    {
        log = "[" + param_.Key + "] - SerialPort can not be opened because there is no matched driverFiles";
        logger.Save(ELogType::Error, log);
        log = LOG_END;
        logger.Save(ELogType::Info, log);
        return false;
    ...
    std::string driverFile = driverFiles[0];
    fileDescriptor_ = ::open(driverFile.c_str(), O_RDWR | O_NOCTTY | O_NONBLOCK);
...
void SerialPort::ReadFromDriver()
{
    ...
    uint64_t previousTimestamp = previousTime.time_since_epoch().count();
    ...
    while (readSwitch_)
    {
        ...
        // 如果相邻两次数据接收时间间隔是否大于5秒钟，设置通信串口的工作状态为异常
        if ((nowTimestamp - previousTimestamp) > 5000000000)
        {
            isNormal_ = false;
        }
        ...
        if ((::ioctl(fileDescriptor_, TIOCINQ, &bytesCount) == 0) && (bytesCount > 0))
        {
            ...
            // 更新上次接收数据时间戳
            std::chrono::time_point<std::chrono::steady_clock> curretnTime = std::chrono::steady_clock::now();
            previousTimestamp = curretnTime.time_since_epoch().count();
        }
    }
}
...
// 搜索通信串口驱动文件
bool SerialPort::SearchDriverFiles(const std::string &path, std::vector<std::string> *driverFiles)
{
    // 初始化搜索结果
    bool result = false;
    ...
    // 根据路径搜索驱动文件
    glob_t fileGlob;
    if (glob(path.c_str(), GLOB_ERR, nullptr, &fileGlob) == 0)
    {
        // 设置搜索结果
        result = true;
        ...
        {
            std::string driverFile = fileGlob.gl_pathv[i];
            driverFiles->emplace_back(driverFile);
        }
    }
    globfree(&fileGlob);
