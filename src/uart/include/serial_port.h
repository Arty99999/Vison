...
#ifndef CUBOT_BRAIN_SERIAL_PORT_H
#define CUBOT_BRAIN_SERIAL_PORT_H
...
public:
    ...
    */
     ...
     *         -<em>false</em> 异常状态\n
     *         -<em>true</em> 正常状态\n
     * @note 如果串口打开之后，连续5秒没有收到数据，工作状态变为异常
     */
    bool IsNormal();
...
private:
    ...
    /**
     * @brief 搜索通信串口驱动文件
     * @param[in] path          驱动文件搜索路径
     * @param[out] driverFiles  驱动文件名
     * @return 驱动文件搜索结果\
     *         -<em>false</em> 搜索失败\n
     *         -<em>true</em> 搜索成功\n
     */
    static bool SearchDriverFiles(const std::string &path, std::vector<std::string> *driverFiles);
};

#endif //CUBOT_BRAIN_SERIAL_PORT_H